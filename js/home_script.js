...
function saveCustomCourse(itemData) {
    try {
        ...
        const newCourse = {
            ...
            provider: extractPlatform(item.link, item.title),
            category: 'custom',
            difficulty: 'beginner',
            duration: 'Self-paced',
            progress: 0,
        ...
        let customCourses = JSON.parse(localStorage.getItem('edumate_custom_courses') || '[]');
        
        const exists = customCourses.some(c => c.link === item.link);
        if (exists) {
            showNotification('Course already saved!', 'info');
            return;
        }
        
        customCourses.push(newCourse);
        localStorage.setItem('edumate_custom_courses', JSON.stringify(customCourses));
...
function viewSavedCourses() {
    ...
    const customCourses = JSON.parse(localStorage.getItem('edumate_custom_courses') || '[]');
    ...
    if (customCourses.length === 0) {
        container.innerHTML = `
            <div class="card" style="grid-column: 1/-1; text-align:center; padding:60px 40px;">
                ...
                <h3 style="margin-bottom:10px;">No Saved Courses Yet</h3>
                <p style="color:var(--muted); margin-bottom:20px; max-width:400px; margin-left:auto; margin-right:auto;">
                    Start searching for courses and click the "Save" button to add them to your collection.
                </p>
                <button class="btn" onclick="quickSearch('programming')" style="margin-top:10px;">
    ...
    container.innerHTML = customCourses.map((course, index) => {
        return `
        <div class="card course-card" style="display:flex; flex-direction:column; height:100%; margin:0; padding:0; overflow:hidden; border:1px solid var(--border); border-radius:12px; position:relative;">
            <div style="position:absolute; top:10px; right:10px; z-index:10;">
                <span style="background:var(--success); color:white; padding:4px 8px; border-radius:12px; font-size:0.7rem; font-weight:600;">
                    Saved
                </span>
            </div>
            <div style="position:relative; height:160px; overflow:hidden; background:#f5f5f5;">
                <img src="${course.image || 'https://via.placeholder.com/300x160/8B5CF6/FFFFFF?text=Course'}" 
                     alt="${course.title}" 
                     style="width:100%; height:100%; object-fit:cover;"
                     onerror="this.src='https://via.placeholder.com/300x160/8B5CF6/FFFFFF?text=Course'">
            </div>
            
            <div style="padding:20px; flex:1; display:flex; flex-direction:column;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <span class="course-category" style="display:inline-block;">${course.provider || 'Custom Course'}</span>
                    <span style="font-size:0.8rem; color:var(--muted);">
                        ${new Date(course.savedDate).toLocaleDateString()}
                    </span>
                </div>
                
                <h3 style="margin:0 0 10px; font-size:1.1rem; line-height:1.4; font-weight:600;">${course.title}</h3>
                ...
                <div style="display:flex; gap:10px; align-items:center; margin-top:auto;">
                    ...
                    <button onclick="removeCustomCourse('${course.id}')" 
                            class="link-btn" 
                            style="flex:0; padding:10px; color:#ef4444; border-color:#ef4444;"
    ...
    header.innerHTML = `
        <div>
            <h3 style="margin:0; display:flex; align-items:center; gap:10px;">
                <span>📚</span> Saved Courses (${customCourses.length})
            </h3>
        </div>
        <button class="btn" onclick="searchCourses()" style="background:var(--primary);">
            ← Back to Search
        </button>
    `;
...
function removeCustomCourse(courseId) {
    ...
    showNotification('Course removed from your list', 'info');
...
function completeRegistration() {
    ...
    if (!username || (users[username] && username !== temp.username)) { 
        alert('ID is required or already taken.'); 
        return; 
    }
...
function viewSavedInternships() {
    ...
    if (container) {
        if (saved.length === 0) {
            container.innerHTML = `
                <div class="card" style="text-align:center;color:var(--muted);padding:40px">
                    <p>No saved Internships yet. Search and save Internships to see them here.</p>
                </div>
            `;
        } else {
            container.innerHTML = saved.map((item, index) => {
                ...
                <div class="card event-card" style="margin-bottom:20px;padding:20px;border:2px solid var(--primary-light);border-radius:8px;background:var(--light)">
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:15px;padding-top:15px;border-top:1px solid var(--border-light)">
                        ...
                        <div style="display:flex;gap:10px">
                            <select onchange="updateInternshipstatus(${index}, this.value)" 
            ...
            `;
            header.innerHTML = `
                <h3 style="margin:0">Saved Internships (${saved.length})</h3>
                <button onclick="loadInternshipsByPosition()" 
...
function loadPlanningData() {
    const plannerData = JSON.parse(localStorage.getItem('edumate_planner'));
    if (plannerData?.semesterData) {
        for (let sem = 1; sem <= 4; sem++) {
            const semData = plannerData.semesterData[`sem${sem}`];
            if (semData) {
                semData.forEach((item, index) => {
                    const courseInput = document.getElementById(`sem${sem}-course${index + 1}`);
                    const creditSelect = document.getElementById(`sem${sem}-credits${index + 1}`);
                    if (courseInput) courseInput.value = item.course || '';
                    if (creditSelect) creditSelect.value = item.credits || '3';
                });
            }
        }
    }
...
function savePlannerData() {
    ...
    let totalCredits = 0;
    const plannerData = { semesterData: {} };
    
    semesters.forEach(sem => {
        let semesterTotal = 0;
        plannerData.semesterData[`sem${sem}`] = [];
        
        for (let i = 1; i <= 3; i++) {
            const creditSelect = document.getElementById(`sem${sem}-credits${i}`);
            if (creditSelect) {
                semesterTotal += parseInt(creditSelect.value) || 0;
            }
            const courseInput = document.getElementById(`sem${sem}-course${i}`);
            if (courseInput && creditSelect) {
                plannerData.semesterData[`sem${sem}`].push({
        ...
        if (totalElement) totalElement.textContent = semesterTotal;
        totalCredits += semesterTotal;
    });
    
    document.getElementById('total-credits').textContent = totalCredits;
    
    const gradProgress = Math.min(Math.round((totalCredits / TOTAL_REQUIRED_CREDITS) * 100), 100);
    const progressBar = document.getElementById('graduation-progress');
    if (progressBar) progressBar.style.width = gradProgress + '%';
    
    plannerData.totalCredits = totalCredits;
    plannerData.gradProgress = gradProgress;
    
    localStorage.setItem('edumate_planner', JSON.stringify(plannerData));
    showNotification('Planner saved successfully!', 'success');
    logActivity('saved', 'Saved academic planner');
}
...
function saveCareerRoadmap() {
    ...
    for (let year = 1; year <= 4; year++) {
        for (let item = 1; item <= 4; item++) {
            const checkbox = document.getElementById(`roadmap-y${year}-${item}`);
            if (checkbox) {
                roadmapData.push({
                    id: `roadmap-y${year}-${item}`,
                    checked: checkbox.checked
                });
            }
        }
    }
    localStorage.setItem('edumate_roadmap', JSON.stringify(roadmapData));
    showNotification('Roadmap updated!', 'success');
    logActivity('updated', 'Updated career roadmap');
}
...
function addCustomCourse() {
    ...
    const newCourse = {
        ...
        image: `https://via.placeholder.com/600x160/8B5CF6/FFFFFF?text=${encodeURIComponent(title.substring(0, 20))}`,
        link: link || '#'
    };
    
    if (!coursesData[category]) coursesData[category] = [];
