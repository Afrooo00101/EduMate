...
// Basic XSS protection helpers
const XSS_FORBIDDEN_PATTERNS = ['<script', 'javascript:', 'onerror=', 'onload='];
...
function isSafeInput(value) {
    ...
    return !XSS_FORBIDDEN_PATTERNS.some(p => lower.includes(p));
}

// Simple login rate limiting (per browser as IP proxy)
...
function getLoginGuardState() {
    try {
        return JSON.parse(localStorage.getItem('edumate_login_guard') || '{}');
    } catch {
        return {};
    }
}
...
function saveLoginGuardState(state) {
    localStorage.setItem('edumate_login_guard', JSON.stringify(state));
}

function getLoginBucket(type) {
...
function canAttemptLogin(type) {
    const state = getLoginGuardState();
    const bucket = getLoginBucket(type);
    const now = Date.now();
...
function registerFailedLogin(type, identifier) {
    const state = getLoginGuardState();
    const bucket = getLoginBucket(type);
    const now = Date.now();
    ...
    if (bucket.count >= LOGIN_LIMIT_MAX_ATTEMPTS) {
        ...
        logSecurityEvent(`Brute force protection triggered for ${type} login`, identifier || 'unknown');
    }
...
function resetLoginAttempts(type) {
    const state = getLoginGuardState();
    if (state[type]) {
        state[type].count = 0;
...
function initializeDemoData() {
    ...
    // Demo feature usage
    const featureUsage = [
        { name: 'Resume Builder', count: 234, icon: 'ðŸ“„' },
        { name: 'Job Search', count: 189, icon: 'ðŸ’¼' },
        { name: 'Courses', count: 167, icon: 'ðŸ“š' },
        { name: 'AI Chat', count: 145, icon: 'ðŸ¤–' },
        { name: 'Scholarships', count: 123, icon: 'ðŸŽ“' },
        { name: 'Faculty Search', count: 98, icon: 'ðŸ‘¨â€ðŸ«' },
        { name: 'Events', count: 87, icon: 'ðŸŽŸï¸' },
        { name: 'Profile', count: 156, icon: 'ðŸ‘¤' },
        { name: 'Resume Templates', count: 112, icon: 'ðŸ“' },
        { name: 'Interview Prep', count: 94, icon: 'ðŸŽ¯' }
    ];
...
function verifyAdminAndProceed() {
    if (window.verifyAdminAndProceed && window.verifyAdminAndProceed !== verifyAdminAndProceed) {
        return window.verifyAdminAndProceed.apply(this, arguments);
    }
    console.warn('verifyAdminAndProceed is handled by admin_backend.js');
}

// Admin Login
function attemptAdminLogin() {
    if (window.attemptAdminLogin && window.attemptAdminLogin !== attemptAdminLogin) {
        return window.attemptAdminLogin.apply(this, arguments);
    }
    console.warn('attemptAdminLogin is handled by admin_backend.js');
}
...
function signOut() {
    ...
    console.warn('signOut is handled by admin_backend.js');
}
...
function getCountryFlag(code) {
    const flags = {
        'EG': 'ðŸ‡ªðŸ‡¬', 'US': 'ðŸ‡ºðŸ‡¸', 'SA': 'ðŸ‡¸ðŸ‡¦', 'AE': 'ðŸ‡¦ðŸ‡ª', 
        'JO': 'ðŸ‡¯ðŸ‡´', 'KW': 'ðŸ‡°ðŸ‡¼', 'GB': 'ðŸ‡¬ðŸ‡§', 'DE': 'ðŸ‡©ðŸ‡ª',
        'FR': 'ðŸ‡«ðŸ‡·', 'CA': 'ðŸ‡¨ðŸ‡¦', 'CN': 'ðŸ‡¨ðŸ‡³', 'RU': 'ðŸ‡·ðŸ‡º',
        'IN': 'ðŸ‡®ðŸ‡³', 'JP': 'ðŸ‡¯ðŸ‡µ', 'KR': 'ðŸ‡°ðŸ‡·', 'BR': 'ðŸ‡§ðŸ‡·'
    };
        ...
        container.innerHTML = blockedIPs.map((ip, index) => `
            <div class="ip-item">
                <div>
                    ...
                    <div style="font-size:0.8rem;color:var(--muted)">${ip.reason} â€¢ ${new Date(ip.date).toLocaleDateString()}</div>
                </div>
    ...
    container.innerHTML = userList.map(user => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px;border-bottom:1px solid var(--border)">
            <div style="display:flex;align-items:center;gap:10px">
                ...
                <div>
                    <div style="font-weight:600">${encodeForHTML(user.name || user.username)}</div>
                    <div style="font-size:0.85rem;color:var(--muted)">${encodeForHTML(user.email || '')}</div>
                </div>
            </div>
        ...
        <tr>
            <td>
                <div style="display:flex;align-items:center;gap:10px">
                    ...
                    <span>${encodeForHTML(user.name || user.username)}</span>
                </div>
            </td>
    ...
    if (adminListElement) {
        adminListElement.innerHTML = ADMIN_EMAILS.map(email => `
            <div class="admin-email-item" style="display: flex; justify-content: space-between; align-items: center; padding: 8px; border-bottom: 1px solid var(--border-color, #e5e7eb);">
                <span>${email}</span>
                <button onclick="removeAdminEmail('${email}')" class="btn-icon-small" style="color: #ef4444;">âœ•</button>
            </div>
        `).join('');
    }
}
