...
class AnalyticsEvent(TimestampMixin, Base):
    ...
    id = Column(UNSIGNED_INTEGER(unsigned=True), primary_key=True, index=True, autoincrement=True)
    student_id = Column(UNSIGNED_INTEGER(unsigned=True), ForeignKey('students.id', ondelete='CASCADE'), nullable=True)
